      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-text-tertiary">{item.vendorName}</p>

            <p className="mt-1 text-xs">
              {!item.available ? (
                <span className="text-text-tertiary">{t("noLongerAvailable")}</span>
              ) : inStock ? (
        ...
        <div className="mt-auto flex items-center gap-4 pt-3">
          ...
          <button
            ...
            onClick={remove}
            disabled={pending}
            className="text-xs text-text-tertiary underline-offset-4 transition hover:text-red-600 hover:underline disabled:opacity-40"
          >
            {t("remove")}
          </button>
        </div>
