...
- Customers can now reorder ("Buy again" on `/account/orders/[id]` re-adds a past order's lines to the cart, best-effort per line via `orderService.reorder` — a discontinued or sold-out item is reported, not blocking the rest). Return and invoice download are still unbuilt; drivers have no UI at all.
