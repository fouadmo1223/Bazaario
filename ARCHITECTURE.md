...
- Customers can now reorder ("Buy again" on `/account/orders/[id]` re-adds a past order's lines to the cart, best-effort per line via `orderService.reorder` — a discontinued or sold-out item is reported, not blocking the rest) and download an invoice (`/account/orders/[id]/invoice`, a print-styled page — "Save as PDF" in the browser print dialog covers the PDF case without a rendering dependency). Returns are still unbuilt; drivers have no UI at all.
